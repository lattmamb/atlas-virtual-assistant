...
export function TailwindcssButtons() {
  const copy = (button: any) => {
    ...
    // Since we don't have reactElementToJSXString in this project,
    // we'll use a simpler approach and just copy a description
    const textToCopy = `Button: ${button.name} - ${button.description || ""}`;
  ...
  return (
    <div className="pb-40 px-4 w-full">
      ...
      >
        <h2 className="text-3xl font-bold mb-2">Button Showcase</h2>
        <p className="text-muted-foreground">Click on any button to copy its code</p>
      </motion.div>
...
export const buttons = [
  ...
  {
    name: "Trinity Button",
    description: "Custom button styled for Trinity Dodge",
    component: (
      <button className="px-8 py-2 rounded-md bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold hover:shadow-lg hover:from-blue-700 hover:to-blue-800 transition duration-200">
        Trinity Dodge
      </button>
    ),
  },
  {
    name: "Atlas Style",
    description: "Button styled to match Atlas Assistant theme",
    component: (
      <button className="px-8 py-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium shadow-md hover:shadow-xl hover:from-blue-600 hover:to-indigo-700 transition-all duration-300">
        Atlas Style
      </button>
    ),
  }
